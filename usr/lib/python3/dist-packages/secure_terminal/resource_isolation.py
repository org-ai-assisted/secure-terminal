...
## breaking no real program. MEM_FRACTION caps a single tab below the whole
## memory budget, leaving headroom for the app plus the other tabs, so one tab's
## runaway is OOM-killed in its own cgroup before the app is ever a candidate.
PIDS_MAX = 4096
MEM_FRACTION = 0.8
...
def effective_mem(base, meminfo=PROC_MEMINFO):
    """Byte budget the MEM_FRACTION applies to: the base cgroup's own memory.max
    when finite, else total RAM. None if neither can be read."""
    try:
...
def create_tab(base, name):
    """Create a limited cgroup <base>/<name> and return its path, or None on any
    failure (fail-open: the tab then runs unlimited). Sets pids.max, and -- when a
    memory budget is knowable -- memory.max plus memory.oom.group so the tab is
    OOM-killed as a unit. All-or-nothing: a partial failure removes the cgroup and
    returns None rather than leave a half-limited tab.
    """
    if base is None:
        return None
