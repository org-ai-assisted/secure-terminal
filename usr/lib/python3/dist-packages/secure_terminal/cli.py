...
Your own keystrokes are forwarded as typed. A raw stdin stream CANNOT reliably
tell typing from a paste: os.read() boundaries are scheduling artifacts, not
key-event boundaries, so fast typing (or keystrokes coalesced by a TTY/SSH
buffer) and a paste can arrive in the exact same shape -- a multi-byte read that
ends in a submit byte. So, unlike the GUI, this wrapper does NOT try to strip a
pasted command's auto-submit: a paste that carries a newline submits, just as it
would in any ordinary terminal (a guess that stripped it would eat the Enter off
fast typing, and a paste split across reads would slip its lone trailing newline
through anyway -- a broken guard is worse than an honest limitation). Auto-submit
protection lives in the GUI, where a real paste event plus bracketed-paste
framing make a paste unambiguous. This wrapper does not judge whether a command
is dangerous either; that is the (planned) hook's job.
"""
        ...
        while True:
            ...
            if stdin_fd in readable:
                ...
                if not keys:
                    ...
                    # Forward keystrokes verbatim: a raw stdin stream cannot tell a
                    # paste from typing (see the module docstring), so there is no
                    # honest byte-level submit strip to do here.
                    os.write(fd, keys)
    finally:
